...
void coverage(const char* filename="finalfitworkspace_uncorrwjets_constantjsf_0.root") {
   ...
   RooMCStudy* mcstudy = new RooMCStudy(*modelc,RooArgSet(*w->var("mjj"),*w->cat("sample")),Constrain(RooArgSet(*w->var("eb"),*w->var("theta"),*w->var("jsf"))),Extended(),Silence(),FitOptions(NumCPU(8), Extended(kTRUE),Minos(*w->var("mjj"))));//

   // Generate and fit nSamples samples of Poisson(nData) events
   mcstudy->generateAndFit(100,data->sumEntries(),kFALSE);

   //_____ Plot and fit Xsec, NPsi2S and NPsi2S/Xsec distributions
   TString binName;
   TCanvas* cdistrJpsi = new TCanvas(Form("cXsecDistr_%s",binName.Data()));
   const RooDataSet* jpsiDataSet = &(mcstudy->fitParDataSet());

   thePoi->setRange("FitXsecWindow",xsec-4*xsecErr,xsec+4*xsecErr);

   RooRealVar meanXsec("meanXsec","average Xsec",xsec,xsec-(0.05*xsec),xsec+(0.05*xsec));
   RooRealVar sigmaXsec("sigmaXsec","average sigmaXsec",xsecErr,0.9*xsecErr,1.1*xsecErr);
   RooGaussian gausJpsi("gausJpsi","gaussian fit",*thePoi,meanXsec,sigmaXsec);
   RooFitResult* fitResultJpsi = gausJpsi.fitTo(*(RooAbsData*)jpsiDataSet,Range("FitXsecWindow"));

   RooRealVar meanXsecNom("meanXsecNom","average Xsec",xsec);
   RooRealVar sigmaXsecNom("sigmaXsecNom","average sigmaXsec",xsecErr);
   RooGaussian gausJpsiNom("gausJpsiNom","gaussian fit",*thePoi,meanXsecNom,sigmaXsecNom);

   RooPlot* frameJpsi = thePoi->frame(Bins(100),Range(xsec-5*xsecErr,xsec+6*xsecErr));
   ...
   cdistrJpsi->Update();
   cdistrJpsi->SaveAs(Form("cXsecDistr_%s.pdf",binName.Data()));
