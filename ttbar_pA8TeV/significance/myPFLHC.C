...
// Author: Émilien Chapon 21/06/2017
...
BEGIN_HTML
<p>
myPFLHC is a concrete implementation of CombinedCalculator 
(the interface class for a tools which can produce both RooStats HypoTestResults and ConfIntervals).  
THIS IS BASED ON RooStats::ProfileLikelihoodCalculator BUT MODIFIED BY ÉMILEN CHAPON.
...
myPFLHC::myPFLHC(RooAbsData& data, RooAbsPdf& pdf, const RooArgSet& paramsOfInterest, 
   ...
   fConstraints(NULL), fDoMinos(false), fDoExtended(false), fNCPU(1)
{
   // constructor from pdf and parameters
   // the pdf must contain eventually the nuisance parameters
}

myPFLHC::myPFLHC(RooAbsData& data,  ModelConfig& model, Double_t size) :
...
RooAbsReal *  myPFLHC::DoGlobalFit() const { 
   ...
   // RooCmdArg constraintsArg = fConstraints.getSize() ? ExternalConstraints(fConstraints) : Constrain(*constrainedParams);
   RooCmdArg ExtArg = fDoExtended ? Extended() : RooCmdArg::none();
   RooAbsReal * nll = pdf->createNLL(*data, CloneData(true), ConditionalObservables(fConditionalObs), Offset(RooStats::IsNLLOffset() ), NumCPU(fNCPU), ExtArg );

   // add constraints
   oocoutI((TObject*)0,Minimization) << "ProfileLikelihoodCalcultor::DoGLobalFit - adding constraints? " << std::endl;
   fConstraints->Print("v");
   if (fConstraints->getSize()>0) {
      oocoutI((TObject*)0,Minimization) << "ProfileLikelihoodCalcultor::DoGLobalFit - really adding constraints " << std::endl;
      RooArgList parcels(*nll);
      TIterator* iter = fConstraints->createIterator();
      RooRealVar *var = (RooRealVar*) iter->Next();
      while (var) {
         parcels.add(*var);
         var = (RooRealVar*) iter->Next();
      }
      nll = (RooAbsReal*) new RooAddition("nllc","nllc",parcels);
   }
   ...
   if (fFitResult) delete fFitResult;
   fFitResult = DoMinimizeNLL(nll, fDoMinos);
...
RooFitResult * myPFLHC::DoMinimizeNLL(RooAbsReal * nll, bool doMinos)  {
   ...
   if (doMinos) {
      oocoutP((TObject*)0,Minimization) << "ProfileLikelihoodCalcultor::DoMinimizeNLL - Running MINOS" << std::endl;
      minim.minos();
   }
...
LikelihoodInterval* myPFLHC::GetInterval() const {
...
HypoTestResult* myPFLHC::GetHypoTest() const {
//    RooAbsPdf* pdf   = fWS->pdf(fPdfName);
//    RooAbsData* data = fWS->data(fDataName);
   RooAbsPdf * pdf = GetPdf();
   RooAbsData* data = GetData(); 
   ...
   for (unsigned int i = 0; i < oldValues.size(); ++i) { 
      ...
      if (mytarget) { 
         ...
         oocoutI((TObject*)0,Minimization) << "ProfileLikelihoodCalcultor::GetHypoTest -  " << poiList[i].GetName() << " has been set constant" << std::endl;
      }
   }
   ...
   if (existVarParams) {
      ...
      RooFitResult * fit2 = DoMinimizeNLL(nll, fDoMinos);
...
void myPFLHC::SetConstraints(RooArgSet* constraints) {
   fConstraints = constraints;
}

void myPFLHC::SetMinos(bool flag) {
   fDoMinos = flag;
}

void myPFLHC::SetExtended(bool flag) {
   fDoExtended = flag;
}

void myPFLHC::SetNCPU(unsigned int ncpu) {
   fNCPU = ncpu;
}
