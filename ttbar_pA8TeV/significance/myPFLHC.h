...
myPFLHC is a concrete implementation of CombinedCalculator (the interface class for a tools which can produce both RooStats HypoTestResults and ConfIntervals).
 ...
 */
 ...
 class myPFLHC : public CombinedCalculator {
   ...
   public:
      ...
      myPFLHC(RooAbsData& data, RooAbsPdf& pdf, const RooArgSet& paramsOfInterest, 
      ...
      /// likelihood function 
      myPFLHC(RooAbsData& data, ModelConfig & model, Double_t size = 0.05);
      ...
      // below is added by EC!
      /// add external constraints to the NLL
      void SetConstraints(RooArgSet* constraints);
      void SetMinos(bool flag);
      void SetExtended(bool flag);
      void SetNCPU(unsigned int ncpu);
   ...
   protected:
    ...
    // minimize likelihood
    static RooFitResult * DoMinimizeNLL(RooAbsReal * nll, bool doMinos); 
    ...
    // below is added by EC!
    RooArgSet* fConstraints;
    bool fDoMinos;
    bool fDoExtended;
    unsigned int fNCPU;
    

    ClassDef(myPFLHC,1) // A concrete implementation of CombinedCalculator that uses the ProfileLikelihood ratio.
